  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "marriage", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "marriage")
    ...
    return NextResponse.json({ imagePath: `/uploads/marriage/${filename}` })
  } catch (error) {
