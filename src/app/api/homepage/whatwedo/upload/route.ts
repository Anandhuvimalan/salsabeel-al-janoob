  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "whatwedo", oldFilename);
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "whatwedo");
    ...
    return NextResponse.json({ imagePath: `/uploads/whatwedo/${filename}` });
  } catch (error) {
