  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "civil", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "civil")
    ...
    return NextResponse.json({ imagePath: `/uploads/civil/${filename}` })
  } catch (error) {
