  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "foriegn", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "foriegn")
    ...
    return NextResponse.json({ imagePath: `/uploads/foriegn/${filename}` })
  } catch (error) {
