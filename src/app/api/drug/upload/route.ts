  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "drug", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "drug")
    ...
    return NextResponse.json({ imagePath: `/uploads/drug/${filename}` })
  } catch (error) {
