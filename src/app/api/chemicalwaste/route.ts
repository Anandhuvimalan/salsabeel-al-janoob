...
const dataFilePath = path.join(process.cwd(), "data", "chemicalwasteData.json")
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to load waste management data" }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ message: "Waste management data updated successfully" })
  } catch (error) {
    console.error("Error saving waste management data:", error)
    return NextResponse.json({ error: "Failed to save waste management data" }, { status: 500 })
  }
}
