  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "allwaste", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "allwaste")
    ...
    return NextResponse.json({ imagePath: `/uploads/allwaste/${filename}` })
  } catch (error) {
