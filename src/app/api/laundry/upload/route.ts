  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "laundry", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "laundry")
    ...
    return NextResponse.json({ imagePath: `/uploads/laundry/${filename}` })
  } catch (error) {
