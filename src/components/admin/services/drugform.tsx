...
export default function DrugForm() {
  const [formData, setFormData] = useState<DrugData | null>(null)
  ...
  const fetchDrugData = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData(data.page_info as DrugData)
        setRecordId(data.id)
      }
    } catch (error) {
      console.error("Error fetching drug data:", error)
      setMessage({ text: "Failed to load drug addiction counseling data. Please refresh the page.", type: "error" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("drug-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("drug-images").upload(fileName, file)

      if (uploadError) throw uploadError

      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("drug-images").getPublicUrl(fileName)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("drug-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("drug").update({ page_info: formData }).eq("id", recordId)

      if (error) throw error

      setMessage({ text: "Drug addiction counseling section updated successfully!", type: "success" })
      await fetchDrugData()
    } catch (error) {
      console.error("Error saving drug data:", error)
      setMessage({ text: "Failed to save drug addiction counseling data. Please try again.", type: "error" })
    } finally {
      setIsSaving(false)
    }
  }
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={inputStyles}
                    placeholder="Enter title color class (e.g., text-red-800)"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter button color class (e.g., bg-red-600)"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter hover button color class (e.g., hover:bg-red-700)"
                />
              </div>
            </div>
          </div>
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                Update Drug Addiction Counseling Section
              </>
            )}
          </Button>
