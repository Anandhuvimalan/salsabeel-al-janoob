...
export default function CivilForm() {
  const [formData, setFormData] = useState<CivilData | null>(null)
  ...
  const fetchCivilData = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData(data.page_info as CivilData)
        setRecordId(data.id)
      }
    } catch (error) {
      console.error("Error fetching civil data:", error)
      setMessage({ text: "Failed to load civil data. Please refresh the page.", type: "error" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("civil-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("civil-images").upload(fileName, file)

      if (uploadError) throw uploadError

      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("civil-images").getPublicUrl(fileName)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("civil-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("civil").update({ page_info: formData }).eq("id", recordId)

      if (error) throw error

      setMessage({ text: "Civil construction section updated successfully!", type: "success" })
      await fetchCivilData()
    } catch (error) {
      console.error("Error saving civil data:", error)
      setMessage({ text: "Failed to save civil data. Please try again.", type: "error" })
    } finally {
      setIsSaving(false)
    }
  }
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                Update Civil Construction Section
              </>
            )}
          </Button>
