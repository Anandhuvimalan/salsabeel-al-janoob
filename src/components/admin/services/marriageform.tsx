...
export default function MarriageForm() {
  const [formData, setFormData] = useState<MarriageData | null>(null)
  ...
  const fetchMarriageData = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData(data.page_info as MarriageData)
        setRecordId(data.id)
      }
    } catch (error) {
      console.error("Error fetching marriage data:", error)
      setMessage({ text: "Failed to load marriage counseling data. Please refresh the page.", type: "error" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("marriage-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("marriage-images").upload(fileName, file)

      if (uploadError) throw uploadError

      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("marriage-images").getPublicUrl(fileName)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("marriage-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("marriage").update({ page_info: formData }).eq("id", recordId)

      if (error) throw error

      setMessage({ text: "Marriage & Family Counseling section updated successfully!", type: "success" })
      await fetchMarriageData()
    } catch (error) {
      console.error("Error saving marriage data:", error)
      setMessage({ text: "Failed to save marriage counseling data. Please try again.", type: "error" })
    } finally {
      setIsSaving(false)
    }
  }
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={inputStyles}
                    placeholder="Enter title color class (e.g., text-teal-800)"
                  />
                </div>
              </div>
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                Update Marriage & Family Counseling Section
              </>
            )}
          </Button>
