...
export default function EducationForm() {
  ...
  const fetchEducationData = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData(data.page_info as EducationData)
        setRecordId(data.id)
      }
    } catch (error) {
      console.error("Error fetching education data:", error)
      setMessage({ text: "Failed to load education data. Please refresh the page.", type: "error" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("education-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("education-images").upload(fileName, file)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("education-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("education").update({ page_info: formData }).eq("id", recordId)

      if (error) throw error

      setMessage({ text: "Educational & Career Guidance section updated successfully!", type: "success" })
      await fetchEducationData()
    } catch (error) {
      console.error("Error saving education data:", error)
      setMessage({ text: "Failed to save educational & career guidance data. Please try again.", type: "error" })
    } finally {
      setIsSaving(false)
    }
  }
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={inputStyles}
                    placeholder="Enter title color class (e.g., text-blue-800)"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter button color class (e.g., bg-blue-600)"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter hover button color class (e.g., hover:bg-blue-700)"
                />
              </div>
            </div>
          </div>
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                Update Educational & Career Guidance Section
              </>
            )}
          </Button>
