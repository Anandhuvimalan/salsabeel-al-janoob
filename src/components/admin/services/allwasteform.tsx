...
export default function WasteManagementForm() {
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("allwaste-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("allwaste-images").upload(fileName, file)

      if (uploadError) throw uploadError

      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("allwaste-images").getPublicUrl(fileName)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("allwaste-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("allwaste").update({ page_info: formData }).eq("id", recordId)
