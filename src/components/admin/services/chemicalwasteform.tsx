...
const defaultData: WasteManagementData = {
  pageInfo: {
    hero: {
    ...
    explanation: {
      ...
      paragraphs: [
        "Salsabeel Al Janoob provides responsible chemical waste management, transforming hazardous materials into environmental renewal. We prioritize community safety and regulatory compliance, offering solutions for industrial and household waste. Our focus is sustainable practices and resource conservation. Effective waste management is resource conservation and pollution reduction.",
        "We manage the complete chemical waste lifecycle, from collection to disposal, using advanced technology and expert professionals. We ensure regulatory adherence and transparency through detailed tracking. Committed to continuous improvement, we provide reliable, cost-effective services that protect the environment and public health, giving clients peace of mind.",
      ],
      imageSrc: "/uploads/chemicalwaste/default-explanation.jpg",
...
export default function ChemicalWasteForm() {
  ...
  const fetchWasteManagementData = async () => {
    ...
    try {
      // Fetch data
      const { data, error } = await supabase.from("chemicalwaste").select("id, page_info").single()
      ...
      if (error) {
        ...
        // Data found
        setFormData(data.page_info as WasteManagementData)
        setRecordId(data.id)
      }
    } catch (error) {
  ...
  const createNewRecord = async () => {
    ...
    try {
      const { data, error } = await supabase.from("chemicalwaste").insert({ page_info: formData }).select("id").single()
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    ...
    if (oldPath && oldPath.includes("chemicalwaste-images")) {
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from("chemicalwaste-images").upload(filePath, file, {
        cacheControl: "3600",
        upsert: false,
      })
      ...
      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("chemicalwaste-images").getPublicUrl(filePath)
  ...
  const handleDeleteImage = async (imagePath: string, section: string, updateState = true) => {
    ...
    const filePathMatch = imagePath.match(/chemicalwaste-images\/(.+)$/)
    ...
    try {
      ...
      const { error } = await supabase.storage.from("chemicalwaste-images").remove([filePath])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (recordId) {
        ...
        const { error } = await supabase.from("chemicalwaste").update({ page_info: formData }).eq("id", recordId)

        if (error) {
          throw error
        }

        setMessage({ text: "Chemical waste management section updated successfully!", type: "success" })
      } else {
        // Create new record
  ...
  return (
    <div>
      {!isAuthenticated && (
        <Alert variant="warning" className="mb-6 bg-amber-950/40 border-amber-700/30">
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                {recordId ? "Update" : "Create"} Chemical Waste Section
              </>
            )}
          </Button>
