...
export default function LaundryForm() {
  const [formData, setFormData] = useState<LaundryData | null>(null)
  ...
  const fetchLaundryData = async () => {
    ...
    try {
      ...
      if (data) {
        setFormData(data.page_info as LaundryData)
        setRecordId(data.id)
      }
    } catch (error) {
      console.error("Error fetching laundry data:", error)
      setMessage({ text: "Failed to load laundry service data. Please refresh the page.", type: "error" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    try {
      ...
      if (oldPath && !oldPath.startsWith("/")) {
        ...
        await supabase.storage.from("laundry-images").remove([oldFileName])
      }
      ...
      // Upload the new file
      const { error: uploadError } = await supabase.storage.from("laundry-images").upload(fileName, file)

      if (uploadError) throw uploadError

      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("laundry-images").getPublicUrl(fileName)
  ...
  const handleDeleteImage = async (imagePath: string, section: string) => {
    ...
    try {
      ...
      if (!imagePath.startsWith("/")) {
        ...
        const { error } = await supabase.storage.from("laundry-images").remove([fileName])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("laundry").update({ page_info: formData }).eq("id", recordId)

      if (error) throw error

      setMessage({ text: "Laundry service section updated successfully!", type: "success" })
      await fetchLaundryData()
    } catch (error) {
      console.error("Error saving laundry data:", error)
      setMessage({ text: "Failed to save laundry service data. Please try again.", type: "error" })
    } finally {
      setIsSaving(false)
    }
  }
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                Update Laundry Service Section
              </>
            )}
          </Button>
