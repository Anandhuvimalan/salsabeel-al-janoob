...
const defaultData: VasthuData = {
  pageInfo: {
    ...
    explanation: {
      ...
      paragraphs: [
        "Our Vasthu Consultancy services blend ancient wisdom with modern design to create balanced and harmonious living and workspaces. We analyze the flow of energy and spatial orientation to optimize your environment for health, prosperity, and peace. With personalized guidance and expert insights, we help you align your space with time-tested principles for maximum positive impact.",
        "Our experienced consultants conduct comprehensive site evaluations, detailed reports, and practical recommendations that empower you to create environments that nurture well-being and success. We work closely with architects, designers, and homeowners to ensure every aspect of your space resonates with balanced energies and aesthetic appeal. Trust our expertise to transform your surroundings into a sanctuary of harmony. Experience the transformative power of truly balanced design.",
      ],
      imageSrc: "/uploads/vasthu/vasthu.avif",
...
export default function VasthuForm() {
  const [formData, setFormData] = useState<VasthuData | null>(null)
  ...
  const fetchVasthuData = async () => {
    setIsLoading(true)
    try {
      // Fetch data
      const { data, error } = await supabase.from("vasthu").select("id, page_info").single()
      ...
      if (error) {
        if (error.code === "PGRST116") {
          ...
          setMessage({
            text: isAuthenticated
              ? "No data found. You can create new data using the form below."
              : "No data found. Please log in to create new data.",
            type: "error",
          })
        } else {
          throw error
        ...
        // Data found
        setFormData(data.page_info as VasthuData)
        setRecordId(data.id)
      }
    } catch (error) {
  ...
  const createNewRecord = async () => {
    if (!isAuthenticated) {
      setMessage({
        text: "You must be logged in to create new data.",
        type: "error",
      })
      return
    }
    ...
    setIsSaving(true)
    setMessage({ text: "Creating new record...", type: "" })

    try {
      const { data, error } = await supabase.from("vasthu").insert({ page_info: formData }).select("id").single()
      ...
      setRecordId(data.id)
      setMessage({ text: "New record created successfully!", type: "success" })
    } catch (error) {
      console.error("Error creating new record:", error)
      setMessage({
        text: `Failed to create new record: ${error instanceof Error ? error.message : "Unknown error"}`,
        type: "error",
      })
    } finally {
      setIsSaving(false)
  ...
  const handleImageUpload = async (file: File, section: string, oldPath = "") => {
    if (!isAuthenticated) {
      setMessage({ text: "You must be logged in to upload images.", type: "error" })
      return null
    }

    // If there's an old image path and it's not a default image, delete it
    if (oldPath && oldPath.includes("vasthu-images")) {
      await handleDeleteImage(oldPath, section, false)
    }
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from("vasthu-images").upload(filePath, file, {
        cacheControl: "3600",
        upsert: false,
      })
      ...
      // Get the public URL
      const { data: publicUrlData } = supabase.storage.from("vasthu-images").getPublicUrl(filePath)
  ...
  const handleDeleteImage = async (imagePath: string, section: string, updateState = true) => {
    if (!isAuthenticated) {
      setMessage({ text: "You must be logged in to delete images.", type: "error" })
      return false
    }

    if (!imagePath || !formData) return false

    // Extract the file path from the URL
    const filePathMatch = imagePath.match(/vasthu-images\/(.+)$/)
    ...
    try {
      ...
      const { error } = await supabase.storage.from("vasthu-images").remove([filePath])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated) {
      setMessage({ text: "You must be logged in to save changes.", type: "error" })
      return
    }
    ...
    try {
      if (recordId) {
        ...
        const { error } = await supabase.from("vasthu").update({ page_info: formData }).eq("id", recordId)

        if (error) {
          throw error
        }

        setMessage({ text: "Vasthu consultancy section updated successfully!", type: "success" })
      } else {
        // Create new record
        await createNewRecord()
      }
    } catch (error) {
      console.error("Error saving vasthu data:", error)
      setMessage({
        text: `Failed to save data: ${error instanceof Error ? error.message : "Unknown error"}`,
        type: "error",
      })
    } finally {
      setIsSaving(false)
  ...
  return (
    <div>
      {!isAuthenticated && (
        <Alert variant="warning" className="mb-6 bg-zinc-900/90 border-amber-600/30">
          ...
          <AlertDescription className="text-amber-300/90">
            You are not logged in. You can view the form but cannot save changes or upload images.
          </AlertDescription>
        </Alert>
      )}
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={inputStyles}
                    placeholder="Enter title color class (e.g., text-purple-800)"
                  />
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter button color class (e.g., bg-purple-600)"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={inputStyles}
                  placeholder="Enter hover button color class (e.g., hover:bg-purple-700)"
                />
              </div>
            </div>
          </div>
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                {recordId ? "Update" : "Create"} Vasthu Section
              </>
            )}
          </Button>
