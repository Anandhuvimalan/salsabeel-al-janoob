  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "waste-management", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "waste-management")
    ...
    return NextResponse.json({ imagePath: `/uploads/waste-management/${filename}` })
  } catch (error) {
