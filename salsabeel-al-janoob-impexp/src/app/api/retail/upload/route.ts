  ...
  try {
    ...
    if (oldImagePath) {
      try {
        ...
        const oldFullPath = path.join(process.cwd(), "public", "uploads", "retail", oldFilename)
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "retail")
    ...
    return NextResponse.json({ imagePath: `/uploads/retail/${filename}` })
  } catch (error) {
